...
interface TrackerReadService {
     ...
     *
     * @param key ключ задачи, например `TREK-42`
     * @return массив пунктов чек-листа
     */
    fun listChecklistItems(key: String): JsonNode

    /**
     * Возвращает записи учёта времени (worklog) задачи.
     *
     * @param key ключ задачи, например `TREK-42`
     * @return массив записей worklog
     */
    fun listWorklogs(key: String): JsonNode

    /**
     * Возвращает список пользователей организации.
     *
     * @param perPage число пользователей на страницу
     * @param page номер страницы, начиная с 1
     * @return страница пользователей со сведениями о пагинации
     */
    fun listUsers(perPage: Int?, page: Int?): PagedResult

    /**
     * Возвращает пользователя по логину или идентификатору.
     *
     * @param id логин или идентификатор пользователя
     * @return объект пользователя
     */
    fun getUser(id: String): JsonNode

    /**
     * Возвращает глобальные поля организации.
     *
     * @return массив полей
     */
    fun listFields(): JsonNode

    /**
     * Возвращает параметры поля по идентификатору или ключу.
     *
     * @param id идентификатор или ключ поля
     * @return объект поля
     */
    fun getField(id: String): JsonNode

    /**
     * Возвращает поля очереди (обязательные и доступные для задач).
     *
     * @param queueId идентификатор или ключ очереди
     * @return массив полей очереди
     */
    fun listQueueFields(queueId: String): JsonNode
}

        ...
        return oauthRestClient.post()
            ...
            .exchange<DeviceAuthorization> { _, response ->
                if (!response.statusCode.is2xxSuccessful) {
                    val errorBody = runCatching { response.bodyTo(TokenResponse::class.java) }.getOrNull()
                    OAuthResponseSupport.requireSuccess(response.statusCode, errorBody)
                }
                val body = response.bodyTo(DeviceCodeResponse::class.java)
                    ?: throw AuthorizationException(
                        "Не удалось получить код устройства: пустой ответ (HTTP ${response.statusCode})",
                    )
                DeviceAuthorization(
        ...
        return oauthRestClient.post()
            ...
            .exchange<TokenSet> { _, response ->
                ...
                if (!response.statusCode.is2xxSuccessful || body?.accessToken == null) {
                    if (!response.statusCode.is2xxSuccessful) {
                        OAuthResponseSupport.requireSuccess(response.statusCode, body)
                    }
                    throw AuthorizationException(
                        "Не удалось обновить токен: ${body?.error ?: "нет access_token"}",
                    )
                }
                body.toTokenSet()
            }!!

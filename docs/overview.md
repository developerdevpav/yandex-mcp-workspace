...
**Yandex MCP Workspace** — набор из двух отдельных локальных MCP-серверов. Tracker и Wiki публикуются как независимые переносимые пакеты, JAR и Docker-образы. Каждый готовый пакет содержит только выбранный сервер и собственную Java Runtime. Серверы общаются с агентом по **stdio** и вызывают REST API Яндекса от имени авторизованного пользователя.

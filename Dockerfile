...
LABEL org.opencontainers.image.title="yandex-mcp-server" \
      org.opencontainers.image.description="MCP server for Yandex Tracker and Yandex Wiki" \
      org.opencontainers.image.source="https://github.com/sorface/yandex-mcp-server" \
...
COPY --from=build /build/target/yandex-mcp-server-*.jar /app/app.jar
